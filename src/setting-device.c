...
static struct _sound_menu_item device_action_menu_its[] = {
...
static int vibrate_type 	 = 0;			/* device_action type */

static int ringtone_count = 0;
static int origin_device_action_level;
...
static int get_device_action_level()
{
	int mode = 1;
	int level = 0;

	//vconf_get_int(VCONFKEY_SETAPPL_NOTI_device_action_LEVEL_INT, &level);

	switch (level) {
	case device_action_LEVEL_NONE_INT:
		mode = device_action_LEVEL_NONE;
		break;
	case device_action_LEVEL_LOW_INT:
		mode = device_action_LEVEL_LOW;
		break;
		/*	case device_action_LEVEL_MID_INT : */
		/*		mode = device_action_LEVEL_MID; */
		/*		break; */
	case device_action_LEVEL_HIGH_INT:
		mode = device_action_LEVEL_HIGH;
		break;
	}
	return mode;
}
...
char *_gl_device_action_title_get(void *data, Evas_Object *obj, const char *part)
{
	...
	if (!strcmp(part, "elm.text")) {
		snprintf(buf, sizeof(buf) - 1, "%s", _(device_action_menu_its[index % VIB_TOP_MENU_SIZE].name));
...
static void _response_device_action_cancel_cb(void *data, Evas_Object *obj, void *event_info)
{
	_haptic_close();

	//vconf_get_int(VCONFKEY_SETAPPL_NOTI_device_action_LEVEL_INT, &vibrate_type);
...
//	vconf_get_int(VCONFKEY_SETAPPL_NOTI_device_action_LEVEL_INT, &origin_device_action_level);
//
//	vibrate_type = (int)data;
//
//	int level = device_action_LEVEL_LOW_INT;
//	int feedback_rate = SETTING_VIB_MEDIUM_RATE;
//	switch (vibrate_type) {
//	case device_action_LEVEL_NONE:
//		level = device_action_LEVEL_NONE_INT;
//		feedback_rate = 0;
//		break;
//	case device_action_LEVEL_LOW:
//		level = device_action_LEVEL_LOW_INT;
//		feedback_rate = SETTING_VIB_WEAK_RATE;
//		break;
//	case device_action_LEVEL_HIGH:
//		level = device_action_LEVEL_HIGH_INT;
//		feedback_rate = SETTING_VIB_STRONG_RATE;
//		break;
//	}
//	vconf_set_int(VCONFKEY_SETAPPL_NOTI_device_action_LEVEL_INT, level);
//	vconf_set_int(VCONFKEY_SETAPPL_TOUCH_FEEDBACK_device_action_LEVEL_INT, level);
//
//	DBG("Setting - feedback level: %d, rate: %d", level, feedback_rate);
//
//	_start_device_action(level, feedback_rate, SETTING_DEFAULT_NOTIFICATION_GENERAL_PREVIEW_VIB);
//	elm_genlist_realized_items_update(g_device_action_type_genlist);
//
//
//	_haptic_close();
//
//	DBG("Setting - device_action level : %d", level);
//
	if (g_device_action_type_genlist != NULL) {
		evas_object_del(g_device_action_type_genlist);
		g_device_action_type_genlist = NULL;
...
static int get_device_action_type()
{
	int type = 1;

	//vconf_get_int(VCONFKEY_SETAPPL_NOTI_device_action_LEVEL_INT, &origin_device_action_level);
	switch (origin_device_action_level) {
	case device_action_LEVEL_NONE_INT:
		type = 2;
		break;
	case device_action_LEVEL_LOW_INT:
