...
void _display_ambient_mode_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	DBG("is_enable_ambient_mode: %d", is_enable_ambient_mode);
	vconf_set_bool(VCONFKEY_SETAPPL_AMBIENT_MODE_BOOL, is_enable_ambient_mode);
...
Evas_Object *_gl_display_check_get(void *data, Evas_Object *obj, const char *part)
{
	...
	if (!strcmp(part, "elm.icon")) {
		...
		if (vconf_get_bool(VCONFKEY_SETAPPL_AMBIENT_MODE_BOOL, &is_enable_ambient_mode) < 0) {
