...
_value_changed_rotary(void *data, Evas_Object *obj, Eext_Rotary_Event_Info *info)
{
	...
	page_data *pd = (page_data *)data;
	int cur_page = 0;
	int val = (int)eext_circle_object_value_get(obj);
	elm_scroller_current_page_get(pd->scroller, &cur_page, NULL);
...
//	_set_volumn(SOUND_TYPE_MEDIA, pd->slider_value[0], VCONFKEY_SETAPPL_MEDIA_SOUND_VOLUME_INT);
//	_set_volumn(SOUND_TYPE_NOTIFICATION, pd->slider_value[1], VCONFKEY_SETAPPL_NOTI_SOUND_VOLUME_INT);
//	_set_volumn(SOUND_TYPE_SYSTEM, pd->slider_value[2], VCONFKEY_SETAPPL_TOUCH_FEEDBACK_SOUND_VOLUME_INT);
...
//		elm_box_pack_end(box, page_layout);
		...
		elm_object_part_text_set(page_layout, "elm.text.slider", "- 3.0 +");
		...
		evas_object_size_hint_min_set(rect, w, h*0.7);
